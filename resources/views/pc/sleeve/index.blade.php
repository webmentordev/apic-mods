<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('sleeve.create') }}" class="py-2 px-4 bg-black text-white font-semibold">Add SLEEVE</a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-[98%] w-full mx-auto sm:px-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <table class="w-full rounded-lg overflow-hidden text-sm">
                            ...
                            @foreach ($sleeves as $item)
                                <tr class="odd:bg-gray-100">
                                    ...
                                    <td class="text-start p-2"><form action="{{ route('sleeve.status.update', $item->id) }}" method="post">
                                        @csrf
                                        @method('PATCH')
                                        @if ($item->is_active)
                                    ...
                                    <td class="text-end p-2">
                                        <form action="{{ route('sleeve.delete', $item->id) }}" method="post" id="form">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <td class="text-end p-2">
                                        <a href="{{ route('update.sleeve', $item->id) }}" class="underline text-indigo-600 cursor-pointer">Update</a>
                                    </td>
                                </tr>
                            @endforeach
                        </table>
                ...
                @else
                    <p class="py-4 text-center">Sleeve data does not exist!</p>
                @endif
            </div>
        </div>
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                form.addEventListener('submit', function (event) {
                    var isConfirmed = confirm('Are you sure you want to delete this sleeve?');
                    if (!isConfirmed) {
