        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="mb-4 text-3xl font-semibold">Add new CASE</h2>
                    @if (session('success'))
                        <x-success :text="session('success')" />
                    @endif
                    <form action="{{ route('case') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="w-full mb-3">
                            ...
                            name="size" :value="old('size')" required :disabled="count($sizes) == 0">
                                @if (count($sizes))
                                    <option value="" selected>-- Please select case size --</option>
