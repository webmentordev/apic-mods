    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Nonvolatile Memory Express (NVMEs)') }}
            </h2>
            <a href="{{ route('nvme.create') }}" class="py-2 px-4 bg-black text-white font-semibold">Add NVME</a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-[98%] w-full mx-auto sm:px-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <table class="w-full rounded-lg overflow-hidden text-sm">
                            ...
                            @foreach ($nvmes as $item)
                                <tr class="odd:bg-gray-100">
                                    ...
                                    <td class="text-start p-2"><form action="{{ route('nvme.status.update', $item->id) }}" method="post">
                                        @csrf
                                        @method('PATCH')
                                        @if ($item->is_active)
                                    ...
                                    <td class="text-end p-2">
                                        <form action="{{ route('nvme.delete', $item->id) }}" method="post" id="form">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <td class="text-end p-2">
                                        <a href="{{ route('update.nvme', $item->id) }}" class="underline text-indigo-600 cursor-pointer">Update</a>
                                    </td>
                                </tr>
                            @endforeach
                        </table>
                ...
                @else
                    <p class="py-4 text-center">NVME data does not exist!</p>
                @endif
            </div>
        </div>
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                form.addEventListener('submit', function (event) {
                    var isConfirmed = confirm('Are you sure you want to delete this nvme?');
                    if (!isConfirmed) {
