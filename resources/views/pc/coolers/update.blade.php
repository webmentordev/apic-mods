        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="mb-4 text-3xl font-semibold">Update Coller</h2>
                    @if (session('success'))
                        <x-success :text="session('success')" />
                    @endif
                    <form action="{{ route('cooler.update', $cooler->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="w-full mb-3">
                            ...
                            name="type" required>
                                <option value="{{ $cooler->type }}" selected>{{ $cooler->type }}</option>
                        ...
                        <div class="w-full mb-3">
                            ...
                            name="aio">
                                <option value="{{ $cooler->aio_type }}" selected>
                                    @if ($cooler->aio_type)
                                        {{ $cooler->aio_type }} - selected
                                    @else
                                        No AIO Type - selected
                                    @endif
                                </option>
                                <option value="">No AIO Type</option>
                                <option value="Air Cooler">Air Cooler</option>
                                <option value="Water Cooler">Water Cooler</option>
                            </x-select>
                        ...
                        <div class="w-full mb-3">
                            <x-input-label for="price" :value="__('Cooler image')" />
